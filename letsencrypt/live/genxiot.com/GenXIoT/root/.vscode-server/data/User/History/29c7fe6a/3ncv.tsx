...
// let the Promise know what you want to do when it resolves
   promise
     .then((data:IDeviceData[]) => dData);
   promise.then((data:IDeviceData[]) => extractData(data));
   ...
   const processData = (data:IDeviceData[]) => {
     ...
     Logging.info("In processData adv : " + (dData))
     data.map((item) => {
     let getData:IDeviceData = JSON.parse(item.toString());
      dXdata.push(getData.deviceData.updatedAt);
       Logging.info("Device Data Y :" + getData.deviceData.payload[varName]);
      dYdata.push(getData.deviceData.payload[varName]);
